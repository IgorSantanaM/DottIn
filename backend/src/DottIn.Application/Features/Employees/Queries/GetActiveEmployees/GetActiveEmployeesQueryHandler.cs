...
{
    public class GetActiveEmployeesQueryHandler(IBranchRepository branchRepository, IEmployeeRepository employeeRepository) : IRequestHandler<GetActiveEmployeesQuery, IEnumerable<EmployeeSummaryDto>>
    {
        public async Task<IEnumerable<EmployeeSummaryDto>> Handle(GetActiveEmployeesQuery request, CancellationToken cancellationToken)
        {
            ...
            var employees = await employeeRepository.GetActiveEmployeesAsync(request.BranchId, cancellationToken);
